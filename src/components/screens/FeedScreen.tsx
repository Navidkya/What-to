...
interface FeedCard {
  id: string;
  type: 'friend_activity' | 'influencer_post' | 'social_moment';
  // friend_activity
  friendName?: string;
...
export default function FeedScreen({ profile: _profile, history: _history, isActive, onToast }: Props) {
  const [cards, setCards] = useState<FeedCard[]>([]);
  const [personPopup, setPersonPopup] = useState<{ name: string; handle?: string; platform?: string; tier?: string; isInfluencer: boolean } | null>(null);
  ...
  const buildFeed = async () => {
    ...
    // Cards de actividade de amigos (mock)
    ...
    // Cards de influencers (Supabase)
    try {
      const infSuggs: InfluencerSuggestion[] = await loadActiveSuggestions();
      const goldSuggs = infSuggs.filter(s => s.influencerTier === 'gold' && s.active).slice(0, 3);
      goldSuggs.forEach((s, i) => {
        feedCards.splice(2 + i * 2, 0, {
          id: `inf-${s.id}`,
          type: 'influencer_post',
          influencer: { name: s.influencerName, handle: s.influencerHandle, tier: 'gold' },
          title: s.title,
    ...
    } catch { /* sem influencers */ }

    // Card de momento social
    feedCards.splice(3, 0, {
      id: 'social-moment-1',
      type: 'social_moment',
      people: ['Ana', 'João', 'Pedro'],
      planItems: ['Sushi', 'Dune', 'Bar'],
      title: 'Plano de noite',
      subtitle: 'Decidiram juntos',
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'#060810', overflowY:'auto', zIndex:10 }}>
      ...
      <div style={{ padding:'12px 16px 100px' }}>
        {cards.map(card => (
          <div key={card.id} style={{ marginBottom:16, borderRadius:20, overflow:'hidden', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)' }}>
            {/* Card header — autor */}
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 14px 0' }}>
              {card.type === 'friend_activity' && (
                <>
                  <div onClick={() => setPersonPopup({ name: card.friendName!, isInfluencer: false })}
                  ...
                  <div style={{ flex:1 }}>
                    <button onClick={() => setPersonPopup({ name: card.friendName!, isInfluencer: false })}
              ...
              {card.type === 'influencer_post' && card.influencer && (
                <>
                  ...
                  <div style={{ fontSize:10, letterSpacing:1, color:'rgba(200,155,60,0.7)', background:'rgba(200,155,60,0.08)', border:'1px solid rgba(200,155,60,0.2)', borderRadius:6, padding:'2px 6px' }}>GOLD</div>
                </>
              )}
            ...
            {personPopup.isInfluencer ? (
              <>
                <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:20 }}>
                  ...
                  <div style={{ marginLeft:'auto', fontSize:10, color:'rgba(200,155,60,0.8)', background:'rgba(200,155,60,0.1)', border:'1px solid rgba(200,155,60,0.25)', borderRadius:8, padding:'3px 8px' }}>GOLD</div>
                </div>
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  ...
                  <button onClick={() => { onToast('✦ A seguir @'+personPopup.handle); setPersonPopup(null); }}
              ...
              <>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {[
                    { label: 'Enviar mensagem', icon: '✉' },
                    { label: 'Ver perfil', icon: '→' },
                    { label: 'Seguir', icon: '+' },
                    { label: 'Silenciar', icon: '○' },
                    { label: 'Deixar de seguir', icon: '×' },
                  ].map((action, i) => (
                    <button key={i} onClick={() => { onToast(action.label); setPersonPopup(null); }}
                      style={{ padding:'12px 16px', background: i===0 ? 'rgba(200,155,60,0.08)' : 'rgba(255,255,255,0.03)', border:`1px solid ${i===0 ? 'rgba(200,155,60,0.2)' : 'rgba(255,255,255,0.07)'}`, borderRadius:12, color: i===4 ? 'rgba(224,112,112,0.7)' : i===0 ? '#C89B3C' : 'rgba(245,241,235,0.7)', fontFamily:"'Outfit',sans-serif", fontSize:13, cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
      ...
      {suggPopup && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)', display:'flex', alignItems:'flex-end', justifyContent:'center', padding:'0 16px 24px', zIndex:200 }}
          ...
          <div style={{ width:'100%', maxWidth:480, background:'rgba(10,12,20,0.97)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:24, padding:'20px 20px 28px' }}
            ...
            <div style={{ fontSize:11, color:'rgba(156,165,185,0.5)', marginBottom:20 }}>{suggPopup.catName}{suggPopup.rating ? ` · ★ ${suggPopup.rating}` : ''}</div>
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
